...
class ExprWithData(object):
    ...
    def monomials(self):
        ...
        #print(self, expr, expr.operator(),operator.mul)
        if 'pow' in str(expr.operator()) or expr.operator() ==None:
            if expr.is_zero():
                return
            yield self, 1
            return
        
        if 'mul' in str(expr.operator()):
            gothrough = [expr]
        elif 'add' in str(expr.operator()):
    ...
    def decompose_monomial(self):
        ...
        (kappa1,3) 
        """
        if 'pow' in str(self.expr.operator()) or self.expr.operator() == None:
            gothrough = [self.expr]
        elif 'mul' in str(self.expr.operator()):
